...
let threshold img threshold =
  List.map  
    (fun row -> 
      List.map (fun v -> if v <= threshold then 0. else 1.) row) 
    img
...
let depict img =
  ...
  let x, y = 
    List.length (List.hd img), 
    List.length img in Graphics.resize_window x y;
...
List.iter depict [mona; mona_threshold; mona_dither] ;;